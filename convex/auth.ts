 ...
 * BULLETPROOF: This query NEVER throws under ANY circumstance.
 * Handles: unauthenticated, missing identity, missing user record,
 * misconfigured auth, malformed user objects, network errors.
  ...
  handler: async (ctx): Promise<{
    ...
    try {
      ...
      try {
        ...
        // getAuthUser threw — unauthenticated, missing identity, or misconfigured
        console.log("[getCurrentUser] getAuthUser failed, returning null");
        return null;
      }

      if (!user) return null;

      // Defensive property access — if user object is malformed, catch below
      const id = user._id;
      if (!id) {
        console.log("[getCurrentUser] user record has no _id, returning null");
        return null;
      }
      ...
      // Absolute last resort — catches anything we missed above
      const msg = outerError instanceof Error ? outerError.message : String(outerError);
      console.log("[getCurrentUser] Unexpected error (returning null):", msg);
      return null;
    }
  },
});
