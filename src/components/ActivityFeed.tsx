...
import {
  useCloudActivityFeed,
  useLocalActivityFeed,
  timeAgo,
  type ActivityEvent,
...
interface ActivityFeedUIProps extends ActivityFeedProps {
  limit: number;
  showAll: boolean;
  onToggleShowAll: () => void;
}

function CloudActivityFeedUI({
  isAuthenticated,
  data,
  weeklyData,
  selectedYear,
  ...
  const events = useCloudActivityFeed(
    isAuthenticated,
    data,
    weeklyData,
    selectedYear,
...
function LocalActivityFeedUI({
  ...
  const events = useLocalActivityFeed(data, weeklyData, selectedYear, limit);
...
const typeColors: Record<string, string> = {
  habit_completed:
    "bg-emerald-100 dark:bg-emerald-900/30 border-emerald-200 dark:border-emerald-800/40",
  habit_missed:
  ...
  streak_milestone:
    "bg-amber-100 dark:bg-amber-900/30 border-amber-200 dark:border-amber-800/40",
  perfect_day:
    "bg-yellow-100 dark:bg-yellow-900/30 border-yellow-200 dark:border-yellow-800/40",
  habit_added:
    ...
    <div className="bg-white dark:bg-gray-800/50 rounded-2xl border border-gray-200/60 dark:border-gray-700/40 overflow-hidden shadow-sm">
      ...
      <div className="divide-y divide-gray-100 dark:divide-gray-800/40">
        {events.map((event) => (
          ...
          >
            <div
              className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 border text-sm ${
        ...
        {events.length === 0 && (
          <div className="p-8 text-center">
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400">
              No activity yet. Start tracking habits to see your feed!
            </p>
          </div>
        )}
      </div>
